// Lab 4b, Using the const Keyword 
...
class Square
{
  const double side;
public:
  Square( const char* const token[] ): side( 0==token[1] ? 0.0 : atof(token[1]) ){}
...
public:
  ...
  : length ( 0==token[1]? 0.0:atof(token[1]) ), width ( 0==token[2]? 0.0:atof(token[2]) ) , height ( 0==token[3]? 0.0:atof(token[3]) ){}
  void output(ostream&) const;
  void xls(ostream&) const;
};
class Cylinder {
...
public:
  ...
  : radius ( 0==token[1]? 0.0:atof(token[1]) ), height ( 0==token[2]? 0.0:atof(token[2]) ){}
  void output(ostream&) const;
  void xls(ostream&) const;
};
...
int main()
{
  ...
  cout << "Lab 4b, Using the const Keyword \n";
